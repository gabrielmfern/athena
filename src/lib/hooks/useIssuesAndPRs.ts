...
export function useIssuesAndPRs(organization: string) {
  ...
  useEffect(() => {
    ...
    async function loadAndFetch() {
      try {
        const cachedIssues = await fetchIssuesAndPRs(organization);
        if (isMounted) {
          setIssues(cachedIssues);
        }

        try {
          const freshIssues = await fetchIssuesAndPRsFromAPI(organization);
