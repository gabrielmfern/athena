...
function App({ organization }: { organization: string }) {
  const { issues, loading, error, refreshIssue } =
    useIssuesAndPRs(organization);
    ...
    <box flexDirection="column" flexGrow={1} padding={1}>
      <box marginBottom={1}>
        <text attributes={TextAttributes.BOLD}>
          {organization} - open issues and pull requests (press q to quit, r to
...
const organization = process.argv[Math.max(process.argv.length - 1, 1)];

if (organization === undefined) {
  console.error(`Usage: athena <organization-name>. ex: athena resend`);
  process.exit(1);
}

const renderer = await createCliRenderer();
const root = createRoot(renderer);
root.render(<App organization={organization} />);
